...
function App() {
  ...
  const addProduct = () => {
    if (newProduct.id && newProduct.name && newProduct.price) {
      axios
        .post(
          "https://everylady-ecommerce.onrender.com/api/products",
          newProduct
        )
        .then((response) => {
  ...
  const deleteProduct = (id) => {
    axios
      .delete(`https://everylady-ecommerce.onrender.com/api/products/${id}`)
      .then(() => {
        setProducts(products.filter((product) => product.id !== id));
        console.log("Product deleted:", id);
      })
      .catch((error) => console.log("Delete Product Error:", error.message));
  };

  const startEdit = (product) => {
    setEditProduct({ ...product });
    setNewProduct({ id: "", name: "", price: "" }); // Clear new product form
  };
  ...
  const saveEdit = () => {
    if (
      ...
      axios
        .put(
          `https://everylady-ecommerce.onrender.com/api/products/${editProduct.id}`,
          editProduct
        )
        .then((response) => {
          setProducts(
        ...
        .catch((error) => console.log("Edit Product Error:", error.message));
    }
  };
            ...
            <div>
              ...
              <div className="mt-4">
                <input
                  type="text"
                  name="id"
                  value={editProduct ? editProduct.id : newProduct.id}
                  onChange={handleInputChange}
                  placeholder="Product ID"
                  className="border p-2 mr-2"
                  disabled={!!editProduct} // Disable ID edit when editing
                />
                <input
                  type="text"
                  name="name"
